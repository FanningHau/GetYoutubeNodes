...
本项目是获取YouTube某频道通过视频介绍分享节点而定义的工作流驱动。

1.  **触发方式**:
    * **定时**: 每天在 `10:00 UTC` (北京时间 18:00) 自动运行。
...
2.  **执行步骤**:
    ...
    * 脚本将新提取的 `xx.txt` 等文件保存在根目录，并将下载的临时文件保存在 `temp_downloads/`。
    * 工作流自动将所有变动 (`git add .`) 提交 (Commit) 并推送 (Push) 回 `main` 分支。
